...
const equipbuttonchromebookcharger = document.getElementById('equipbuttonchromebookcharger')
const equipflamethrower = document.getElementById('equipflamethrower')
const equipsofa = document.getElementById('equipsofa')
...
const characters = {
    ...
    knee: {
      name: "Blue Grinch",
      coinMultiplier: 10000,
      speedMultiplier: 1.3,
      image: new Image(),
    },
    peter: {
      name: "Peter Griffin",
      coinMultiplier: 50000,
      speedMultiplier: 1,
      image: new Image(),
    }
};
...
characters.diddy.image.src = "https://assets.onecompiler.app/42wswghpg/42wymus2a/image_2024-11-06_172932889.png"
characters.ksi.image.src = "https://assets.onecompiler.app/42wswghpg/42wymus2a/image_2024-11-06_173333693.png"
characters.trump.image.src = "https://assets.onecompiler.app/42wswghpg/42wymus2a/image_2024-11-06_204536004.png"
characters.knee.image.src = "https://assets.onecompiler.app/42wswghpg/42wymus2a/image_2024-11-07_172726260.png"
characters.peter.image.src = "https://assets.onecompiler.app/42wswghpg/42wymus2a/image_2024-11-07_214422798.png"
...
const biggGemConvertButton = document.createElement('button');
biggGemConvertButton.textContent = "Convert 100M Coins to 1000 Gems";
document.body.appendChild(biggGemConvertButton);

biggGemConvertButton.addEventListener('click', () => {
    if (coins >= 100000000) {
        coins -= 100000000;
        gems += 1000;
        alert(`Successfully converted! You now have ${gems} gems!`);
    } else {
        alert("You need 100M coins to convert to gems!");
    }
});

const bigggGemConvertButton = document.createElement('button');
bigggGemConvertButton.textContent = "Convert 1B Coins to 10000 Gems";
document.body.appendChild(bigggGemConvertButton);

bigggGemConvertButton.addEventListener('click', () => {
    if (coins >= 1000000000) {
        coins -= 1000000000;
        gems += 10000;
        alert(`Successfully converted! You now have ${gems} gems!`);
    } else {
        alert("You need 1B coins to convert to gems!");
    }
});

const mConvertButton = document.createElement('button');
mConvertButton.textContent = "Convert 10,000 gems to 1 diamond";
document.body.appendChild(mConvertButton);

mConvertButton.addEventListener('click', () => {
    if (gems >= 10000) {
        gems -= 10000;
        diamonds += 1;
        alert(`Successfully converted! You now have ${diamonds} diamonds!`);
    } else {
        alert("You need 10000 gems to convert to a diamond!");
    }
});

const mmConvertButton = document.createElement('button');
mmConvertButton.textContent = "Convert 100,000 gems to 10 diamonds";
document.body.appendChild(mmConvertButton);

mmConvertButton.addEventListener('click', () => {
    if (gems >= 100000) {
        gems -= 100000;
        diamonds += 10;
        alert(`Successfully converted! You now have ${diamonds} diamonds!`);
    } else {
        alert("You need 100000 gems to convert to 10 diamonds!");
    }
});
...
const chromeli = document.createElement('li');
chromeli.textContent = "Chromebook Charger - 5000 Gems";
shopItemsList.appendChild(chromeli);

const buychrome = document.createElement('button');
buychrome.textContent = "Buy Chromebook Charger (5000 Gems)";
shopPanel.appendChild(buychrome);

buychrome.addEventListener('click', () => {
    if (gems >= 5000) {
        gems -= 5000;
        addToInventory("Chromebook Charger");
        updateInventoryDisplay();
        updateShopDisplay();
    } else {
        alert("Not enough gems to buy Chromebook Charger!");
    }
});

const flameli = document.createElement('li');
flameli.textContent = "Flamethrower - 1 Diamond";
shopItemsList.appendChild(flameli);

const buyflamer = document.createElement('button');
buyflamer.textContent = "Buy Flamethrower (1 Diamond)";
shopPanel.appendChild(buyflamer);

buyflamer.addEventListener('click', () => {
    if (diamonds >= 1) {
        diamonds -= 1;
        addToInventory("Flamethrower");
        updateInventoryDisplay();
        updateShopDisplay();
    } else {
        alert("Not enough gems to buy Flamethrower!");
    }
});

const sofali = document.createElement('li');
sofali.textContent = "Sofa - 50 Diamonds";
shopItemsList.appendChild(sofali);

const buysof = document.createElement('button');
buysof.textContent = "Buy Sofa (50 Diamonds)";
shopPanel.appendChild(buysof);

buysof.addEventListener('click', () => {
    if (diamonds >= 50) {
        diamonds -= 50;
        addToInventory("Sofa");
        updateInventoryDisplay();
        ...
        alert("Not enough gems to buy Sofa!");
    }
});
...
const buydiddy = document.createElement('button');
buydiddy.textContent = "Buy Diddy (500 Gems)";
characterShopPanel.appendChild(buydiddy );

buydiddy.addEventListener('click', () => {
    if (gems >= 500 && !unlockedCharacters.includes('diddy')) {
        gems -= 500;
        unlockedCharacters.push('diddy');
        currentCharacter = characters.diddy;
        player.speed *= currentCharacter.speedMultiplier;
        buydiddy.textContent = "Select Diddy";
    } else if (unlockedCharacters.includes('diddy')) {
        currentCharacter = characters.diddy;
        player.speed = 5 * currentCharacter.speedMultiplier;
    }
});

const buyksi = document.createElement('button');
buyksi.textContent = "Buy KSI (1000 Gems)";
characterShopPanel.appendChild(buyksi );

buyksi.addEventListener('click', () => {
    if (gems >= 1000 && !unlockedCharacters.includes('ksi')) {
        gems -= 1000;
        unlockedCharacters.push('ksi');
        currentCharacter = characters.ksi;
        player.speed *= currentCharacter.speedMultiplier;
        buyksi.textContent = "Select KSI";
    } else if (unlockedCharacters.includes('ksi')) {
        currentCharacter = characters.ksi;
        player.speed = 5 * currentCharacter.speedMultiplier;
    }
});

const buytrump = document.createElement('button');
buytrump.textContent = "Buy Trump (2500 Gems)";
characterShopPanel.appendChild(buytrump );

buytrump.addEventListener('click', () => {
    if (gems >= 2500 && !unlockedCharacters.includes('trump')) {
        gems -= 2500;
        unlockedCharacters.push('trump');
        currentCharacter = characters.trump;
        player.speed *= currentCharacter.speedMultiplier;
        buytrump.textContent = "Select Trump";
    } else if (unlockedCharacters.includes('trump')) {
        currentCharacter = characters.trump;
        player.speed = 5 * currentCharacter.speedMultiplier;
    }
});

const buyknee = document.createElement('button');
buyknee.textContent = "Buy Blue Grinch (5 diamonds)";
characterShopPanel.appendChild(buyknee );

buyknee.addEventListener('click', () => {
    if (diamonds >= 5 && !unlockedCharacters.includes('knee')) {
        diamonds -= 5;
        unlockedCharacters.push('knee');
        currentCharacter = characters.knee;
        player.speed *= currentCharacter.speedMultiplier;
        buyknee.textContent = "Select Blue Grinch";
    } else if (unlockedCharacters.includes('knee')) {
        currentCharacter = characters.knee;
        player.speed = 5 * currentCharacter.speedMultiplier;
    }
});

const buypeter = document.createElement('button');
buypeter.textContent = "Buy Peter Griffin (25 diamonds)";
characterShopPanel.appendChild(buypeter );

buypeter.addEventListener('click', () => {
    if (diamonds >= 25 && !unlockedCharacters.includes('peter')) {
        diamonds -= 25;
        unlockedCharacters.push('peter');
        currentCharacter = characters.peter;
        player.speed *= currentCharacter.speedMultiplier;
        buypeter.textContent = "Select Peter Griffin";
    } else if (unlockedCharacters.includes('peter')) {
        currentCharacter = characters.peter;
        player.speed = 5 * currentCharacter.speedMultiplier;
    }
});
...
function createDebugPanel() {
    ...
    diamondsInput.placeholder = 'Enter diamonds amount';
    diamondsInput.style.display = 'none';
    ...
    const adddiamondButton = document.createElement('button');
    adddiamondButton.textContent = 'Add Diamonds';
    adddiamondButton.style.display = 'none';
    ...
    submitButton.addEventListener('click', () => {
        if (passwordInput.value === 'dadnameharold') {
            ...
            diamondsInput.style.display = 'block';
            addCoinsButton.style.display = 'block';
            addGemsButton.style.display = 'block';
            adddiamondButton.style.display = 'block';
    ...
    adddiamondButton.addEventListener('click', () => {
        diamonds += parseInt(diamondsInput.value) || 0;
    });
    ...
    debugPanel.appendChild(diamondsInput)
    debugPanel.appendChild(addCoinsButton);
    debugPanel.appendChild(addGemsButton);
    debugPanel.appendChild(adddiamondButton);
...
function toggleEquipAK() {
  if (equippedWeapon === "AK-47") {
    equippedWeapon = null;
    equipButtonak.textContent = "Equip AK-47";
  } else if (inventory.includes("AK-47")) {
    equippedWeapon = "AK-47";
    equipButtonak.textContent = "Unequip AK-47";
  }
  updateInventoryDisplay();
}
...
function togglecharger() {
    if (equippedWeapon === "Chromebook Charger") {
        equippedWeapon = null;
        equipbuttonchromebookcharger.textContent = "Equip Chromebook Charger";
    } else if (inventory.includes("Chromebook Charger")) {
        equippedWeapon = "Chromebook Charger";
        equipbuttonchromebookcharger.textContent = "Unequip Chromebook Charger";
    }
    updateInventoryDisplay();
}

function toggleflamer() {
    if (equippedWeapon === "Flamethrower") {
        equippedWeapon = null;
        equipflamethrower.textContent = "Equip Flamethrower";
    } else if (inventory.includes("Flamethrower")) {
        equippedWeapon = "Flamethrower";
        equipflamethrower.textContent = "Unequip Flamethrower";
    }
    updateInventoryDisplay();
}

function togglesofa() {
    if (equippedWeapon === "Sofa") {
        equippedWeapon = null;
        equipsofa.textContent = "Equip Sofa";
    } else if (inventory.includes("Sofa")) {
        equippedWeapon = "Sofa";
        equipsofa.textContent = "Unequip Sofa";
    }
    updateInventoryDisplay();
}
...
function update() {
        ...
        if (canHitSquare) {
            let coinReward = 5;
              if (equippedWeapon === "Bottle") {
                ...
                coinReward *= 100000;
              } else if (equippedWeapon === "Chromebook Charger") {
                coinReward *= 250000;
    ...
    ctx.font = "16px Arial";
    ctx.fillText(`Diamonds: ${abbreviateNumber(diamonds)}`, 300, 60);
...
equipbuttonchromebookcharger.addEventListener('click', togglecharger);
equipsofa.addEventListener('click', togglesofa);
equipflamethrower.addEventListener('click', toggleflamer);
